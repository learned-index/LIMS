...
public:
    ...
    KNN_IO();
    KNN_IO(Point &, mainRef_Point &, priority_queue<pair<double, unsigned int> > &,CalCirclePos &, vector<CalCirclePos> &, unsigned, int8_t (*)[20000], unsigned, int &);
    ...
    void DoRangeQueryForKNN(unsigned, vector<vector<int> > &, unsigned long long, mainRef_Point &, priority_queue<pair<double, unsigned int> > &, int8_t (*)[20000], int &);
};

KNN_IO::KNN_IO(){}

KNN_IO::KNN_IO(Point &KNN_pt, mainRef_Point &mainRef_point, priority_queue<pair<double, unsigned int> > &KNNRes_queue, CalCirclePos &mainRefPtCircle, vector<CalCirclePos> &ref_query, unsigned k, int8_t (*arr)[20000], unsigned clu_id, int &page)
{
    ...
    DoRangeQueryForKNN(i, rank_list, queryRange, mainRef_point, KNNRes_queue, arr, page);
}

void KNN_IO::DoRangeQueryForKNN(unsigned i, vector<vector<int> > &rank_list, unsigned long long queryRange, mainRef_Point &mainRef_point, priority_queue<pair<double, unsigned int> > &KNNRes_queue, int8_t (*arr)[20000], int &page){
    if (i == rank_list.size() - 1) {
        ...
        if(find_queryLower>=find_queryUpper){
            if(find_queryLower == find_queryUpper && find_queryLower >= query_lower && find_queryUpper <= query_upper){
                ...
                for(int x = start_page; x <= end_page; ++x){
                    if(arr[clu_id][x] == 1)
                        continue;

                    vector<vector<double> > pt_page;
                    string fileName = "./data/cluster_" + to_string(clu_id) + "_" + to_string(x);
                    ...
                    for(int s = 0; s < size_page; ++s){
                        double dis_pt_qryPt = CaculateEuclideanDis(pt_page[s], queryPoint);
                        if(KNNRes_queue.size() < k){
                            pair<double, unsigned int> elem(dis_pt_qryPt, mainRef_point.iValuePts[x*Constants::PAGE_SIZE+s].id);
...
int KNN_IO::DoublingSearch(vector<double> &disList, bool flag, double target, int start){
...
int KNN_IO::BinarySearch(vector<double> &disList, double target, int low, int high){
    int middle = 0;
    while(low <= high) {
		middle = (high - low) / 2 + low;
...
double KNN_IO::CaculateEuclideanDis(vector<double> &coordinate_a, Point &point_b){
    double total = 0.0;
